...
const SchemDetails = () => {
  ...
  const getTableData = async () => {
    ...
    try {
      const response = await fetch(
        `${url}:${port}/api/values/GetSchemDetails?Comp=${comp}&FY=${fy}CustCode=${selectedOption.value}`
      );
      const result = await response.json();
      console.log(result);
      if (result.Status === 1) {
            ...
            <div className="card-body p-2 mt-3">
              <div className="row">
                ...
                <div className="col-lg-3 ms-auto d-flex justify-content-between">
                  <div
                    className={
                      userData[0].ExcelExp == 1 ? "input-blocks" : "d-none"
                    }
                  >
                  ...
                  <div>
                    <button
                      onClick={exportToPDF}
                      className="pdf-btn pdf-btn-primary ms-auto"
                    >
